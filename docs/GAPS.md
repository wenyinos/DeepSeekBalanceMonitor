...
|---|---|---|
| 抬头（3–5 行） | 「描述 v2.0.2 的 Python-Windows 运行时架构」 | 纯 Rust；版本号 2.1.1 |
...
1. **版本号**：1.x 文档写到 v2.0.2（Python 线末版），本分支是 **2.1.1**（新增桌面小工具，
   详见 `WIDGET.md`）。
...
**截至 2.1 的状态**（2026-09-15 逐条对代码核实）：4.1–4.6 都还没有做；**4.7 的热力图已经做了**
——不在主程序里，而在 2.1 新增的桌面小工具（`dsmon-ui/src/widget/charts.rs`，12 周 × 7 天的额度
活动图，可按订阅切换），主程序的订阅页仍是每日用量折线；**时段分布柱状图两条路径都没有**。
