...
1.x 有、2.0 尚未实现的功能。每条给出 1.x 行为、2.0 现状、移植要点与建议。
...
### 4.7 热力图与时段分布图 —— 建议：弃（中）

- **1.x 行为**：看板除折线外还有 GitHub 风格热力图（180 天/30 天）与时段分布柱状图。
- **2.0 现状**：只有趋势折线（余额页）与每日用量折线（订阅页）。
- **取舍**：属于展示丰富度；egui_plot 画热力图需要自绘，成本不低。
