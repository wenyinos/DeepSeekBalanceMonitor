...
|---|---|---|
| 抬头（3–5 行） | 「描述 v2.0.2 的 Python-Windows 运行时架构」 | 纯 Rust；版本号 2.1.4（1.x 已到 1.5.0） |
...
1. **版本号**：1.x 文档写到 v2.0.2（Python 线末版），本分支是 **2.1.4**（新增桌面小工具，
   详见 `WIDGET.md`）。
...
FlashDuty 自己的 `Open API` 组件，于是状态恒为「正常」；现在读厂商自己的 `status.deepseek.com` 的
**RSS 事件流**（`/feed.rss`）——页面本身是 Next.js 外壳，HTML 里带的是**历史**事件的组件快照，照它取
最严重会把早已解决的故障报成正在发生。该地址还要求 TLS 握手把后量子混合组 `X25519MLKEM768` 排在第一
位，所以本版的 TLS provider 是手动选的（`platforms::http_client`），细节见 `AGENTS.md`。
