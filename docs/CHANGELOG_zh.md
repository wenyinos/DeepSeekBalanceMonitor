...
- **同一页的卡片统一尺寸**：卡片原先各自按内容决定高度，于是订阅页里三窗口的计划（OpenCode Go、Command
  Code、GLM）比旁边两窗口的计划高出约六十点，一排看起来是两根不同高的柱子。现在整页共用一个高度：一个标题
  加上每窗口一行，按页面上最多的那个计划算 —— 数字是**从真实渲染的卡片量出来的**，不是估的（本程序自己的
  间距下标题 41 点、每窗口 77 点；若按 egui 默认间距量，每行会短 15 点），另留几点余量应对宿主字体差异。
  这是**下限而非写死的高度**：万一某张卡真的需要更多空间，它会自己长高、看得出来；换成上限则会把多出的内容
  悄悄裁掉
- **连接状态卡也加入它所在的那一排**：所有没有状态页的平台（Kimi、MiniMax、StepFun、OpenRouter）的余额页
  第三张卡，从来没有设置另外两张卡共用的高度，于是除了 DeepSeek 那一页，这些页面的顶部一排一直不齐，而且
  它总是最矮的那张。另外，读取失败的原因现在**单行显示、悬停看全文**（与订阅卡一致）：在固定高度下，长错误
  会把文字顶出卡片面板
...
- **订阅窗口现在会说自己烧得多快，并按这个速度预告「重置前就会用完」**：卡片原本只有「已用百分比」
  与「距重置时间」，它们回答「现在到哪了」，却不回答「照这个速度会在重置前用光吗」——而后者才是额度
  唯一值得打扰人的问题。现在每个窗口多一行速度（每天多少个百分点），来自当前周期内已记录的读数；
  周期从最近一次重置算起，首次见到时还没重置过的计划则从最早那条读数算起。闲置时间**算进**速度里，
  因为窗口是按钟表重置、不是按使用重置。速度会赶在重置之前用光的窗口，卡片上直接写明，并且每个周期
  提醒一次
- **服务状态有了历史**：每次读数都在记录 DeepSeek 的状态，而界面此前只画最新那一个。历史页的图表上方
  多了一条状态带，与图表使用同一批读数：状态相同的连续读数合成一段、各一种颜色，下方给出「可用读数里
  健康占多少」。读不到状态的读数**不计入**这个比例，另起一段单列——那是本程序的故障、不是厂商的，
  算成故障就把账记错了人。整天读不到状态会显示成一段灰色，2026-09-16 到 20 日那四天正是如此
- **有新版本会告诉你**：程序以 .deb/.rpm/MSI 分发，自己没有任何更新通道，所以一个构建可能与它的修复
  只差一个版本、却一直停在那里——服务状态这次就是这样：修复早就写好，机器上装的构建早于它，而没有任何
  地方说明。现在每天向 release 页查一次最新版本，有更新的会显示在设置页版本号旁，并按版本提醒一次。
  这次查询只有一次请求、可以关掉，查询失败则什么也不说

### 修复

- **界面与小工具不再等数据库**：轮询原先**握着快照锁**写历史、读历史，于是界面的下一帧、以及小工具的
  请求（应答就是从同一份历史拼出来的）都可能被拖成等 SQLite。现在这把锁只在发布读数时短暂持有。线索
  来自一次失败的发布构建：测试里的 widget 用例从共享测试库读历史拼应答，而 `cargo test` 并行跑用例时，
  SQLite 5 秒的 busy timeout 拖过了它 3 秒的读超时；碰同一个库的用例现在也排队进入
- **状态页的读数一直没能到达界面**：feed 前面的边缘节点会重置「**第一个**密钥交换组不是后量子混合组
  `X25519MLKEM768`」的握手——只报 `X25519` 会被重置，把混合组排在后面也一样，服务端一句话都不说。
  而 reqwest 0.11 内置的 rustls 0.21 完全没有后量子组，于是每次抓取都倒在握手上，界面只能显示「未知」
  ——同一时刻 `curl`、`wget`、`go` 读同一个 feed 却毫无问题。改用 reqwest 0.12 与 rustls 0.23 的
  `prefer-post-quantum`，把混合组排到第一位（浏览器就是这么发的）。（2026-09-20 定位，线索是
  `balance_history` 里连续四天的 `unknown`。）抓取失败现在会写一行日志，状态变化时才写，不再只以
  「未知」的形式沉默
...
- **小工具显示「今日消耗」**（4.5 的后一半）：余额卡在余额下面多一行今日已花掉的金额，数据经契约的
  `today_spend` 字段送达（含平台、币种与金额）。主程序的提醒与它是同一个数，口径一致

- **单日消耗过快提醒**（4.5）：当日 DeepSeek 消耗超过设定的线值（默认 0 = 关闭）时，托盘图标变橙并
  提醒一次；线值在设置页，与低额提醒并列。图标新增第五种状态，六套配色都补了颜色
- **峰谷切换提醒**（4.4）：DeepSeek 的谷时优惠开始或结束各提醒一次（按北京时间 09:00–12:00 /
  14:00–18:00 的工作日判定，周末与午休都算谷时）。默认开，设置页可关
- **OCGo 周/月剩余精化**（4.1）：端点的周/月只有整数百分比、5h 却是金额，而池是已知的
  （$12 / $30 / $60）——「本步内花掉的金额 ÷（池 ÷ 100）」把粗百分比细化到小数，1.x 的核心展示特性
- **MiniMax 请求重试**（4.2）：主机偶尔掐断连接（`UNEXPECTED_EOF`），一秒后重试即可，最多三次；
  被拒的密钥不重试。顺带不再复用可能已死的长连接
...
- **精化的百分比现在真的显示出来**：`history::refined_percent` 把 OpenCode Go 的周/月从整数百分比
  ...
  新的 `history::format_percent` 让整数保持整数、精化值保留两位，订阅页与小工具的额度卡共用它
- **升级后的第一次启动不再认不出密钥**：`storage::open_db()` 的建表与补列是「看了再改」，而升级后
  第一次启动时，轮询线程与界面的密钥查询会同时开库——两边都去补 `window` 列（一边被告知已存在），
  并互相抢写锁（`database is locked`）。界面那条抢输后一个密钥也没读到，弹出「尚未配置」并跳到设置页；
  而密钥一直好好地躺在库里、也解得开，重启即恢复。现在整段由一个进程内锁串起来，补列另外容忍
  `duplicate column name`
- **全新安装也能开库了**：补 `window` 列原先排在 `subscription_history` 建表**之前**，新建的库会
  直接报 `no such table`。现在建表在前、补列居中、引用该列的索引在最后

## Rust v2.1.1 (2026-09-15)

### 新增

- **桌面小工具也能开机自启**了：它有自己的会话启动项（`widget_auto_start`，默认开），与主程序那条
  互相独立——可以只让小工具随登录出现（那时主程序没跑，它会显示断开提示并自动重连），也可以让它等
  主程序来拉起。用小工具的 ✕ 关闭时，这条自启项一并撤掉，否则下次登录它又会自己回来

### 修复

- 首次生成密钥文件时，若另一个调用者正在创建同一个文件，Windows 会把这件事报成「拒绝访问」或
  「找不到文件」而不是「文件已存在」，于是加密直接失败。现在三种回答被当作同一件事处理：再去看一眼
  文件（写入未完成时会等它写完），只有确实什么都没有时才把最初的错误报出来。这个问题让 v2.1.0 的
  Windows 发布连续三次失败在同一个测试上

## Rust v2.1.0 (2026-09-15)

### 新增：桌面小工具 `dsmon2-widget`

- 独立程序：无边框、半透明、可置顶的竖排卡片面板，显示各平台的余额与订阅额度。**只从主程序取数**
  ——本机回环 `127.0.0.1:18964` 上的只读接口，不查任何平台 API、不读数据库、不碰任何密钥。主程序
  没在跑时给出断开提示并每 10 秒自动重连，旧数据保留并整体灰显
- 卡片三种：**每个已配置的余额平台各一张**（余额、消耗速率、余额曲线；还没有消耗历史的平台只显示
  余额与曲线）、每个订阅各一张（每个额度窗口两行：`窗口名 …… 重置 3h 56m 86%` + 进度条）、最下方
  一张**额度活动**（12 周 × 7 天热力图，标签在 `全部` 与各订阅之间点击切换，默认「全部」＝各订阅
  按日期相加；标签只用一个词的短名，全名在悬停提示里）
- 标题条六个按钮：明暗切换、不透明度四档（25 / 50 / 75 / 90，档位由圆的填充程度表示，不写数字）、
  置顶（置顶时图标用强调色高亮）、立即刷新（断开或已有查询在飞时置灰）、打开主程序设置、关闭
- 窗口：**宽度锁定**（一列卡片加宽没有意义），**高度拖下方两个角改变**；位置与高度写回配置并在下次
  启动恢复。标题条不参与拖动——拖拽区压在按钮上会把点击吞掉
- **小工具自己的托盘**：菜单「显示/隐藏桌面小工具」「启动主程序」「退出」，左键点击即切换显示
- **小工具自己的图标**（主程序的鲸鱼 + 强调色角标），窗口与托盘共用一张；Linux 用 X11 的 `Utility`
  窗口类型、Windows 用 `with_taskbar(false)`，都不进任务栏与窗口列表
- 单实例：D-Bus 名 `…-widget` / 互斥体 `Local\DeepSeekBalanceMonitorWidget`，重复启动只把已有窗口唤到
  前台；与主程序的名字互不干扰
- 中英双语、亮暗双主题与主程序共用同一份文案与配色，在任一边改，另一边两秒内跟上

### 新增：主程序侧

- **本地只读接口**：`GET /widget-status?days=1|7|30` 与 `GET /check`（触发一次查询并立即返回当前
  快照），只绑回环、不做鉴权也不传凭据。契约写在 `docs/INTERFACES.md`，Python 版照此实现即可，
  小工具一行不改
- 托盘菜单新增「显示/隐藏桌面小工具」，勾选状态即 `widget_enabled`；主程序启动时按它拉起小工具，
  小工具自己的关闭按钮会把这个开关一并关掉
- **打包各出各的包**：主程序与小工具在 Linux 各一份 `.deb`/`.rpm`，在 Windows 各一个 `.msi`。两个
  MSI 装进同一个目录（`Program Files\DeepSeek Balance Monitor`），因为两个可执行文件必须在一起才能
  互相拉起；但它们是可以分别安装、升级、卸载的两个产品

### 变更

- `config.json` 改为**原子写**（临时文件 + `rename`）。从这一版起有两个进程读写同一份配置，
  非原子写会让人读到写了一半的内容，而读不出来会被当成损坏文件改名保存——用户设置就这么丢了
- `widget_opacity` 的档位改为 0.25 / 0.50 / 0.75 / 0.90（原来只有 0.5 与 1.0 两档，最高档并不是
  全不透明）

### 修复

- 小工具标题栏的六个按钮恢复可点。整条标题栏原先兼作拖动把手，而拖拽区压在按钮下面会把按下事件抢走
  （指针一动窗口管理器就接管了指针），于是六个按钮全部失效——窗口还能拖，只有按钮不响应。现在拖动
  只由左/右/下三条窄边负责，标题条不参与任何交互
- 补齐三项「契约里写了、小工具没做」（服务端一直是完整的，缺的是消费端）：载荷 `version` 的**版本
  协商**（主程序用更新的格式时，面板顶部出现红色提示「主程序版本过新，请升级小工具」）；刷新按钮的
  **进行态与禁用态**；配置里的 `widget_show_trend` 现在真的生效（关闭后不再画余额折线）
- 小工具的 Windows 托盘补上两处：缺少的 `use std::sync::Arc`（Windows 分支只由 CI 编译，这一处是
  人工审读发现的），以及「系统是否真的收下了图标」的检查——图标库对注册失败不报错，现在会写日志
