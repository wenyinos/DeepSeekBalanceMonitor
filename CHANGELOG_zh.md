...
- Windows 设置窗口重做：原来的四个标签页（账号 / 常规 / 历史 / 订阅）改为左侧导航加右侧卡片页面。左侧四个按钮切换页面，当前页按钮加粗；每页用带边框的卡片分组（账号页：凭据、OpenCode Go、Command Code，卡片标题即各平台的密钥标签；常规页：左列查询与代理，右列通用与图标外观）。控件、读写的配置项与保存 / 取消行为完全不变；窗口尺寸由 520×720 调整为 760×680 以容纳导航列。

### 说明

- Linux 版本本次没有改动，版本号跟随共用的 `rust-v*` tag，使一个 release 里的两个包同号发布。
