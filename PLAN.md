...
| 托盘左键 | 打开主窗口（原定切换小工具，小工具推迟后改为打开窗口） |
| 托盘右键 | 打开主窗口 / 立即刷新 / 明暗切换 / 设置 / 退出 |
...
**悬浮小工具**（已决定推迟，代码不保留；配置字段 `widget_*` 留在 schema 里）：无边框半透明卡片，始终置顶（可关闭）、尺寸两档（紧凑约 160×80 仅余额 / 标准约 220×110 含更新时间）、透明度 50%–100% 可调、带迷你趋势线（近 24 小时走向）；显示 DeepSeek 主币种余额 + 状态色点；可拖动、双击唤出主窗口、右键菜单；默认右上角、默认不显示。

推迟的原因之一：Wayland 不允许应用设置或读取窗口位置，「默认右上角」与「位置记忆」在 Wayland 会话下只能由合成器决定，需要 wlr-layer-shell 才能真正实现。
